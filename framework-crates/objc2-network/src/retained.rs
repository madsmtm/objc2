...
///
/// [`NWRetained`] strongly references or "retains" the given object `T`, and
...
/// [`NWObject`], i.e. any Network object.
///
///
...
/// that this type uses the underlying Network primitives `nw_retain` /
/// `nw_release` instead of `objc_retain` / `objc_release` for performance,
/// and to avoid depending on the Objective-C runtime if not needed.
///
/// You can also view `NWRetained<T>` as the Network equivalent of
...
///
/// Weak references are not supported without the Objective-C runtime, for
/// that you need to convert to `objc2::rc::Retained`.
///
#[cfg_attr(
    not(feature = "objc2"),
...
/// to use e.g. `NWRetained<NWProxyConfig>` as-if it you had a
/// `&NWProxyConfig`. Note that having a `NWProxyConfig` directly is not
...
impl<T: ?Sized + NWObject> NWRetained<T> {
    ...
    /// # Safety
    ///
    /// The pointer must be a valid and live object according to Network, and
    ...
    /// Unchecked conversion to another Network object.
    ///
...
impl<T: ?Sized + NWObject + objc2::Message> From<objc2::rc::Retained<T>> for NWRetained<T> {
    ...
    ///
    /// This only works if the type is a Network object (implements the
    /// [`NWObject`] trait).
    ///
    ...
    fn from(obj: objc2::rc::Retained<T>) -> Self {
        ...
        // SAFETY: `T` is bound by `NWObject`, so we know that the type is a
        // Network object, and hence we know that it will respond to
