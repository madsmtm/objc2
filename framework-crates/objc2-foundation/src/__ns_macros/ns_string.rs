...
macro_rules! __ns_string_static {
    ($inp:ident) => {
        ...
        const UTF16_FULL: (&[u16; $inp.len()], usize) = {
            ...
            let mut iter = $crate::__ns_macros::EncodeUtf16Iter::new($inp);
            let mut written = 0;
        ...
        static CFSTRING: $crate::__ns_macros::CFConstString = unsafe {
            if $crate::__ns_macros::is_ascii_no_nul($inp) {
                // This is technically an optimization (UTF-16 strings are
                // always valid), but it's a fairly important one!
                $crate::__ns_macros::CFConstString::new_ascii(
                    &$crate::__ns_macros::__CFConstantStringClassReference,
                    &ASCII,
                )
            } else {
                $crate::__ns_macros::CFConstString::new_utf16(
                    &$crate::__ns_macros::__CFConstantStringClassReference,
                    &UTF16,
                )
            }
