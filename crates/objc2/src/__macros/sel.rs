...
/// `"unstable-core-darwin-objc"` feature which uses the nightly-only
/// `darwin_objc` language feature. This experimental language feature
/// implements the Objective-C static selector ABI directly in the Rust compiler
/// and should work in more if not all cases. Using
/// `"unstable-core-darwin-objc"` requires `darwin_objc` to be enabled in every
/// crate that uses this macro, which can be achieved in `objc2` crates by
/// enabling their own `"unstable-core-darwin-objc"` features and in your own
/// crates by adding `#![feature(darwin_objc)]`.
/// See [rust-lang/rust#145496] for tracking the `darwin_objc` language feature.
///
/// `"unstable-static-sel"` and `"unstable-static-sel-inlined"` take precedence
/// over `"unstable-core-darwin-objc"`.
///
///
...
#[doc(hidden)]
#[macro_export]
#[cfg(not(any(feature = "unstable-core-darwin-objc", feature = "unstable-static-sel")))]
...
#[macro_export]
#[cfg(all(
    feature = "unstable-core-darwin-objc",
