...
use std::{ffi::CString, mem::ManuallyDrop, ptr::NonNull, time::Duration};

use objc2::rc::Retained;

use crate::{AsRawDispatchObject, QualityOfServiceClassFloorError, TryFromDurationError};

use super::{ffi::*, function_wrapper, QualityOfServiceClass};
...
impl Queue {
    ...
    /// Returns system main queue.
    pub fn main_queue() -> Self {
        // Safety: object cannot be null.
        let inner = unsafe {
            Retained::from_raw(dispatch_get_main_queue())
    ...
    pub fn global_queue(identifier: GlobalQueueIdentifier) -> Self {
        ...
        // Safety: object cannot be null.
        let inner = unsafe {
            Retained::from_raw(object).expect("dispatch_get_global_queue shouldn't fail!")
        };
...
impl Drop for Queue {
    fn drop(&mut self) {
        ...
        if !self.is_global_queue {
            // Safety: safe because there is no access to manually drop afterwards.
