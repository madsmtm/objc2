...
impl Encoding {
    ...
    /// considering everything is packed, i.e. no alignment is computed.
    pub fn size(&self) -> usize {
        Helper::new(self).size(NestingLevel::new())
    }

    /// Computes the raw signature string of the object corresponding to the
    /// block taking `args` as inputs and returning `ret`.
    ///
    /// See [`objc2::encode::block_signature_string`] for more details. Prefer
    ...
    pub fn block_signature_string(args: &[Self], ret: &Self) -> String {
        // TODO: alignment?
        let arg_sizes = args.iter().map(Encoding::size).collect::<Vec<_>>();
    ...
    fn test_block_signature_string() {
        for (args, ret, val) in [
            ...
            (
                ...
                &Encoding::String,
                if cfg!(target_pointer_width = "64") {
                    "*64@?0(ThisOrThat=Si)8{ThisAndThat=QjDAB}12"
                } else {
                    "*60@?0(ThisOrThat=Si)4{ThisAndThat=QjDAB}8"
                },
            ),
            (
                &[
            ...
            assert_eq!(Encoding::block_signature_string(args, ret), val);
        }
    }
}
